...
const getMySchedule = async (filters: any, options: IPaginationOptions, user: IAuthUser) => {
...
const getAllFromDB = async (filters: IDoctorScheduleFilterRequest, options: IPaginationOptions) => {
  const cacheKey = doctorScheduleCacheKeys.list(filters, options);
  ...
  const result = await redisHelper.getOrSetCache(
    ...
    async () => {
      ...
      const resultData = await prisma.doctorSchedules.findMany({
        include: {
          doctor: true,
          schedule: true,
        },
